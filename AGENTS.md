...
│   └── database/                   # SQL 迁移脚本
├── wechat/miniprogram/
│   ├── yansongda/                  # 主微信小程序（Bun）
│   └── totp/                       # TOTP 独立微信小程序（Bun）
...
- 必须提交的锁文件：`Cargo.lock`、`bun.lock`（yansongda / totp）、`oh-package-lock.json5`（华为）。
...
- **多包管理器并存**：后端 cargo，微信主小程序 Bun，TOTP 小程序 Bun，华为 ohpm。
